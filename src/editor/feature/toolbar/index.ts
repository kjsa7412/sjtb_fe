...
class ToolbarView implements PluginView {
  ...
  constructor(ctx: Ctx, view: EditorView, config?: ToolbarFeatureConfig) {
    ...
    this.#tooltipProvider = new TooltipProvider({
      ...
      debounce: 20,
      offset: 10,
      shouldShow(viewTmp: EditorView) {
        const { doc, selection } = viewTmp.state
        ...
        const {activeElement} = viewTmp.dom.getRootNode() as ShadowRoot | Document
        const isTooltipChildren = content.contains(activeElement)

        const notHasFocus = !viewTmp.hasFocus() && !isTooltipChildren

        const isReadonly = !viewTmp.editable
